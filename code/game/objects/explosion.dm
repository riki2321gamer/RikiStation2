/proc/explosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog = 1, ignorecap = 0, flame_range = 0 ,silent = 0, smoke = 1)
	if(devastation_range>0)
		message_admins("Explosion.dm : Devastation Range is too high ([devastation_range] > 0), defaulting to 0.")
		devastation_range = 0
	if(heavy_impact_range>0)
		message_admins("Explosion.dm : Heavey impact range is too high ([heavy_impact_range] > 0), defaulting to 0.")
		heavy_impact_range = 0
	if(light_impact_range>10)
		message_admins("Explosion.dm : Light impact range is too high ([light_impact_range] > 10), defaulting to 10.")
		light_impact_range = 10
	if(flash_range>40)
		message_admins("Explosion.dm : Flash range too high ([flash_range] > 40), defaulting to 40.")
		flash_range = 40
	if(flame_range>10)
		message_admins("Explosion.dm : Flame range too high ([flame_range] > 10), defaulting to 10.")
		flame_range = 10
