...
/mob/living/carbon/proc/devour_mob(mob/living/carbon/C, devour_time = 5)
	C.visible_message("<span class='danger'>[src] is attempting to vore [C]!</span>", \
					"<span class='userdanger'>[src] is attempting to vore you!</span>")
	//src.say("")
	if(!do_mob(src, C, devour_time))
		return
	if(pulling && pulling == C && a_intent == INTENT_GRAB) //&& grab_state >= GRAB_AGGRESSIVE GRAB_AGGRESSIVE
		C.visible_message("<span class='danger'>[src] vores [C]!</span>", \
						"<span class='userdanger'>[src] vores you!</span>")
		C.forceMove(src)
		stomach_contents.Add(C)
		playsound(C, 'sound/ambience/vored.ogg', 50, 1)
		C.say("help")

		add_logs(src, C, "devoured")
		src.resize = 1.1 //biggen the idiot who just ate Someone because We Need More Space Right?
