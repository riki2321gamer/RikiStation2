...
/mob/living/simple_animal/hostile/alien/pet
	name = "Lusty"
	melee_damage_lower = 0
	melee_damage_upper = 0
	a_intent = INTENT_HELP
...
/mob/living/simple_animal/hostile/alien/pet/AttackingTarget()
	if(istype(target, /atom/movable))
		var/atom/movable/M = target
		var/numberex = rand(1,4)
		if(numberex!=1)
		else
			visible_message("[src] caresses \the [target].")
		//M is player, do anything to them
		return TRUE