...
/obj/item/weapon/fidgetspinner/attack_self(mob/user)
	if(mph < 1)
		mph = mph + 10
		user.emote("me",1,"is spinning the fidget spinner at [mph] MPH!")
		while (loop > 0)
			if(mph > 0)
				while (number < 8)
					number = number + 1
					icon_state = "fidget[number]"
					sleep(1)
				number = 1
				if(mph < 0)
					mph = 0
