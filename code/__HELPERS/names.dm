...
/proc/church_name()
	...
	name += pick("Fuck", "Nooya", "LDA", "Hunter", "Vix")

	if (prob(20))
		name += " Get Spaced"

	name += " " + pick("H Carrell", "iDot", "A Ipad", "GS1029", "Cool", "Great")
...
/proc/religion_name()
	...
	name += pick("IEDs", "mob of monkeys", "Block", "Blockstation", "/noober/", "Woah hey guys", "EB Games", "Number one")
	name += pick(" asm", " Sux", " HQ", " TM", " Trivago", " Explosion", " Copyrighted by Sony", " : LDA Syndicate")
...
/proc/new_station_name()
	...
	for(var/holiday_name in SSevents.holidays)
		if(holiday_name == "El Funnio Meme")
			random = 13
...
/proc/syndicate_name()
	...
	// Prefix
	name += pick("RPed", "", "Prjizevired", "SupperMemed", "Spergy", "Succ", "Sux", "Nooby")
	...
	if (prob(80))
		...
		// Full
		if (prob(60))
			name += pick("Syndicate", "Noodle", "TM", "Games")
		// Broken
		else
			name += pick("Gay", "Cool", "Sux", "LDA", "H Carrell", "Ipad", "Trademarked", "Riki")
			name += pick("", "-")
...
/proc/change_station_name(designation)
	if(config && config.server_name)
		world.name = "[config.server_name] presents to you: [designation]"
	else
