...
/client/verb/wiki()
	...
	set hidden = 1
	if(config.wikiurl)
		if(alert("This will open discord in your browser. Are you sure?",,"Yes","No")=="No")
			return
...
/client/verb/rules()
	set name = "youtube channel"
	set desc = "See riki2321's youtube channel."
	set hidden = 1
	if(config.rulesurl)
		if(alert("This will open riki2321's youtube channel in your browser. Are you sure?",,"Yes","No")=="No")
			return
		src << link(config.rulesurl)
	else
		to_chat(src, "<span class='danger'>The youtube URL is not set in the server configuration.</span>")
	return
